...
public class ShipmentItemDomainService {
    /**
     * Kiểm tra tính hợp lệ của số lượng cập nhật
     */
    public void validateUpdateQuantity(ShipmentItem item, Integer newQuantity) {
        if (newQuantity == null) {
        ...
        if (newQuantity > item.getQuantity()) {
            throw new IllegalArgumentException("Số lượng không được vượt quá số lượng yêu cầu");
        }
    }
     ...
     */
    public void updatePickedStatus(ShipmentItem item, Integer pickedQty) {
        // không được nhặt quá số lượng
    ...
    public void updatePackedStatus(ShipmentItem item, int packedQty) {
        ...
        if (packedQty < 0) {
            throw new IllegalArgumentException("Số lượng đóng gói không được âm");
        }
        if (packedQty == item.getQuantity()) {
            item.setNote("Hoàn thành đóng gói (Fully Packed)");
        }
    }
    ...
    // .map(item ->
    // item.getWeightGram().multiply(BigDecimal.valueOf(item.getQuantity())))
